...
def naive_model(dat, weight = None,
                ...
                step = 0.0001, lmda = 0.0001, factorDim = 50,
                alpha = 0.5,
    ...
    if u_init == None or bu_init == None or bi_init == None:
        ...
        mean_arts = np.array(mean_art_dict.values())
        mean_user = np.array([np.median(art_value)
    ...
    B_i = np.array(mean_arts - U)

    B_u = B_u.reshape((N_user, 1))
    B_i = B_i.reshape((N_arts, 1))
    P_u = (np.random.sample((N_user, factorDim)) - 0.5)/10
    Q_i = (np.random.sample((N_arts, factorDim)) - 0.5)/10
