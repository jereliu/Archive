...
os.chdir("/Users/Jeremiah/GitHub/CS-181-Practical-3/Script/Jeremiah")
...
plays_pd = pd.DataFrame(plays_array, columns=['Number of Play'])
plays_pd["log(Play Count)"] = np.log(plays_pd['Number of Play'])

plays_pd_quant1 = plays_pd.quantile(np.array(range(100)).astype("float")/100)
plays_pd_quant2 = \
    plays_pd.quantile(0.99+np.array(range(1, 10)).astype("float")/1000)
plays_pd_quant3 = \
    plays_pd.quantile(0.999+np.array(range(1, 11)).astype("float")/10000)

plays_pd_q = pd.concat([plays_pd_quant1, plays_pd_quant2, plays_pd_quant3])
plays_pd_q["Quantile"] = plays_pd_q.index*100
plays_pd_q.columns = ["Play Count", "Quantile"]
plays_pd_q["log(Play Count)"] = np.log(plays_pd_q["Play Count"])

hist = ggplot(aes(x='Quantile', y = "log(Play Count)"),
              data = plays_pd_q) + geom_point() + xlim(-5, 105)
ggsave("hist_freq_qq.pdf", hist)

hist = ggplot(aes(x = "log(Play Count)"),
              data = plays_pd) + geom_histogram()
ggsave("hist_freq_log.pdf", hist)
